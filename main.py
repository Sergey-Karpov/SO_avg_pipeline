...
# Создание pipeline для ОБУЧЕНИЯ (используем справочники)
...
# Удаляем city (теперь он не нужен)
...
# Определяем ВХОДНЫЕ признаки для предсказания (8 колонок, БЕЗ city!)
...
# Создаем pipeline для ПРЕДСКАЗАНИЯ (не требует city)
...
def predict_from_input(input_data):
    ...
    input_data: pandas DataFrame с колонками (8 признаков, БЕЗ city!):
        ...
        - cereals: float - количество круп
        - milk: float - количество молока
